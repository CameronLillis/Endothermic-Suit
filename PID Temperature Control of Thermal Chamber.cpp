...
float TR_C; // Remote temperature in deg C, must be normalized in argument for
            // PI Controller
...
void Potentiometer(bool action); // Read the potentiometer setting as an analog
                                 // input and display as 0 to 100%
...
float TR_C_filterFunction(float timeConstant, float processGain, float blockIn,
                          float intervalTime);
float TL_C_filterFunction(float timeConstant, float processGain, float blockIn,
                          float intervalTime);
float DerivativefilterFunction(float timeConstant, float processGain,
                               float blockIn, float intervalTime);
void setup() {
...
void loop() {
  ...
  if (Auto == true) {
    heaterSetting = SetpointGenerator(); // this establishes the PID Setpoint
                                         // ... range is 0 to 110 deg C
    contOutNorm = PID_output(
        C_Variable / 500, heaterSetting / 500, propBand, integralTime,
        derivativeTime, 2000,
        controlAction); // normalized 0 to 1.0 of PID controller output ...
                        // PB=800, Tint = 32, Td=0, interval =100msec
    if (controlAction == true) {
      analogWrite(
          10,
          255 * contOutNorm); // converts PID controller output toa PWM value
  ...
  if (Manual == true) {
    ...
    if (controlAction == true) {
      analogWrite(
          10, ((float)potA1 / 1023 * 255 *
               2.90)); // 2.90 added to compensate for 1.66 volts at top of pot
    } else if (controlAction == false) {
...
void SerialPlotter(float controllerOutput) {
  if (Auto == true & Manual == false) {
    ...
    Serial.print(
        ((float)potA1 *
         0.4887585)); // plot value of potA1, either Set Point or Manual
    Serial.print(",");
    Serial.print(TR_C_Filtered); // Plot remote LM35 which is controlled
                                 // variable 0 to 500
    Serial.print(",");
    ...
    Serial.print(0.0); // plot value of potA1, either Set Point or Manual
    Serial.print(",");
    Serial.print(int((float)potA1 * 0.09775 *
                     2.90)); // plot value of potA1, either Set Point or Manual
    Serial.print(",");
    Serial.print(TL_C_Filtered); // Plot remote LM35 which is controlled
                                 // variable 0 to 500
    Serial.print(",");
    Serial.println(TR_C_Filtered); // Plot remote LM35 which is controlled
                                   // variable 0 to 500
  }
  ...
  // Serial.print(",");
  // Serial.println(tempRemote);// Plot remote LM35 which is controlled variable
  // 0 to 110 Serial.print(","); Serial.print("TR_C_Filtered = ");
  // Serial.print(",");
...
void LM35_A2() {
  ...
  TR_C_Filtered = TR_C_filterFunction(
      5, 1.0, TR_C, 2000); // filter time constant is first argument in seconds
...
float SetpointGenerator() // Set Point 0 to 110 dg C
{
  ...
  //{
  setTemp =
      (float)potA1 * 0.4887585; // generates a temp setpoint of 0 to 500 deg C
...
float PID_output(float process, float setpoint, float Prop, float Integ,
  ...
  Limiter_Switch = 1;
  // delay(Interval);  // Interval in msec is delta t in the integral and
  ...
  // output or 0% output Prevents integral windup, where controller keeps
  // integrating when controller output can no longer affect the process.
  ...
  Integral = Integral + 100 / Prop / Integ * Er * deltaT *
                            Limiter_Switch; // Integral calculator
  derivative =
      100 / Prop * deriv * (Er - Olderror) / deltaT; // Derivative calculator
  ...
  if (Cont >
      1) // limit controller output between 0.0 and 1.0 a normalized value
    Cont = 1;
...
float TR_C_filterFunction(float timeConstant, float processGain, float blockIn,
  ...
  blockOut =
      blockOut + (intervalTime / 1000 / (timeConstant + intervalTime / 1000)) *
                     (processGain * blockIn - blockOut);
  return blockOut;
}
